...
def init_db():
    ...
    if not db_exists:
        ...
        # Set pragmas to help with concurrency
        ...
        # Insert sample users
        sample_users = [
            ('Alice', 'Smith', 'asmith', 'P@$$wOrd1', 'alice.smith@gmail.com', '2024-03-01', 0),
            ('Bob', 'Johnson', 'bobbyj', 'MySecurePwd', 'bob.johnson@gmail.com', '2024-05-10', 0),
            ('Carol', 'Williams', 'carolw', 'StrongerPass', 'carol.williams@gmail.com', '2024-08-22', 0),
            ('David', 'Brown', 'davidb', 'VerySafe123', 'david.brown@gmail.com', '2025-01-05', 0),
            ('Emily', 'Davis', 'emilyd', 'AnotherSecure', 'emily.davis@gmail.com', '2025-04-15', 0)
        ]
        ...
        # Insert collections - just the 3 requested categories
        collections = [
            ('Books', 'Book collection'),
            ('Shoes', 'Shoe collection'),
            ('Figurines', 'Figurine collection')
        ]
        
        for collection in collections:
            c.execute("INSERT INTO Collections (CollectionName, Description) VALUES (?, ?)", collection)
        
        # Insert sample items
        items = [
            # Books
            (1, 'Harry Potter Series', 'Complete set of Harry Potter books', 120.00, 5),
            (1, 'The Lord of the Rings', 'Fantasy novel by J.R.R. Tolkien', 25.99, 10),
            (1, 'To Kill a Mockingbird', 'Classic novel by Harper Lee', 15.50, 7),
            (1, 'Pride and Prejudice', 'Novel by Jane Austen', 12.99, 8),
            (1, 'The Great Gatsby', 'Classic American novel', 14.99, 6),
            (1, '1984', 'Dystopian novel by George Orwell', 13.50, 9),
            
            # Shoes
            (2, 'Nike Air Max', 'Running shoes', 89.99, 4),
            (2, 'Adidas Superstar', 'Classic sneakers', 75.00, 6),
            (2, 'Converse Chuck Taylor', 'High-top canvas shoes', 55.00, 8),
            (2, 'Vans Old Skool', 'Skate shoes', 65.00, 5),
            (2, 'New Balance 574', 'Casual athletic shoes', 79.99, 4),
            (2, 'Puma Suede Classic', 'Classic suede sneakers', 69.99, 7),
            
            # Figurines
            (3, 'Marvel Avengers Set', 'Set of 6 Avengers figurines', 99.99, 3),
            (3, 'Star Wars Darth Vader', 'Collectible Darth Vader figurine', 45.99, 5),
            (3, 'Harry Potter Wizards', 'Set of Harry Potter characters', 85.00, 4),
            (3, 'Game of Thrones Dragons', 'Dragon figurine collection', 129.99, 2),
            (3, 'Pokemon Starter Set', 'Original starter Pokemon figurines', 59.99, 6),
            (3, 'Disney Princesses', 'Collection of Disney princess figurines', 79.99, 4)
        ]
        ...
        # Add sample user item relationships
        # Format: UserID, ItemID, DateAdded, Quantity
        user_items = [
            (1, 1, today, 1),  # Alice has Harry Potter
            (1, 14, today, 2),  # Alice has Star Wars figurine
            (1, 7, today, 1),   # Alice has Nike shoes
            
            (2, 2, today, 1),   # Bob has LOTR
            (2, 8, today, 2),   # Bob has Adidas shoes
            (2, 15, today, 1),  # Bob has Harry Potter figurines
            
            (3, 3, today, 1),   # Carol has To Kill a Mockingbird
            (3, 9, today, 1),   # Carol has Converse shoes
            (3, 16, today, 3),  # Carol has GoT dragons
            
            (4, 4, today, 2),   # David has Pride and Prejudice
            (4, 10, today, 1),  # David has Vans shoes
            (4, 17, today, 2),  # David has Pokemon figurines
            
            (5, 5, today, 1),   # Emily has Great Gatsby
            (5, 11, today, 1),  # Emily has New Balance shoes
            (5, 18, today, 2)   # Emily has Disney Princess figurines
        ]
...
class UserTab(QWidget):
    def __init__(self, user_id):
        ...
        # Don't keep a persistent connection
        self.setup_ui()
        self.load_collections()
        self.load_items()
        self.load_my_items()
    ...
    def load_items(self):
        # Create a new connection each time to avoid locking issues
        conn = sqlite3.connect(DB_NAME, timeout=10)
        c = conn.cursor()
    ...
    def add_to_my_items(self, item_id):
        try:
            # Create a new connection with timeout to avoid locked database
            conn = sqlite3.connect(DB_NAME, timeout=10)
            c = conn.cursor()
    ...
    def closeEvent(self, event):
        # No need to close a persistent connection
        super().closeEvent(event)
...
def main():
    ...
    # Delete the database file if it exists to ensure clean initialization
    if os.path.exists(DB_NAME):
        try:
            os.remove(DB_NAME)
            print(f"Existing database {DB_NAME} deleted for clean initialization")
